...
if (-not (Test-Path -Path "admin\views\products.bak2.php")) {
    Copy-Item -Path "admin\views\products.php" -Destination "admin\views\products.bak2.php"
}

# We'll manually create the fixed content and overwrite the file
$fixedContent = @'
<?php
...
# Write fixed content to a temp file to ensure it's proper
$fixedContent | Set-Content -Path "admin\views\products.fixed.php" -Encoding UTF8

# If the file was created successfully, replace the original
if (Test-Path -Path "admin\views\products.fixed.php") {
    Copy-Item -Path "admin\views\products.fixed.php" -Destination "admin\views\products.php" -Force
    Write-Host "File has been completely rebuilt with proper formatting!"
} 