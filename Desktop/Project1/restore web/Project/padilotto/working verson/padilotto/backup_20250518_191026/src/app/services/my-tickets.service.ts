...
//  apiUrl = 'http://localhost:3000';
  ...
  getAllWinningTickets(): Observable<any> {
    ...
    return this.http.get<any>(`${this.apiUrl}/api/simple/winning-tickets`, { headers })
      .pipe(
        tap(response => {
          console.log('Raw winning tickets response:', response);
          if (response && !response.success) {
            console.warn('API reported failure:', response.message || 'No error message provided');
          }
        }),
        map(response => {
          if (response && response.success && Array.isArray(response.tickets)) {
            console.log(`Processing ${response.tickets.length} winning tickets`);
            return response.tickets;
          } else if (Array.isArray(response)) {
            console.log(`Processing ${response.length} winning tickets (direct array)`);
            return response;
          }
          console.warn('No valid winning tickets array found in response');
          return [];
        }),
        catchError(error => {
          console.error('Error fetching winning tickets:', error);
          if (error.status === 404) {
            console.error('API endpoint not found. Verify the winning tickets endpoint is correctly implemented.');
          }
          return of([]);
        })
      );
  ...
  async createTransaction(transactionData: any): Promise<any> {
    try {
      const response = await this.http.post(`${this.apiUrl}/api/transactions`, transactionData).toPromise();
      return response;
    } catch (error) {
