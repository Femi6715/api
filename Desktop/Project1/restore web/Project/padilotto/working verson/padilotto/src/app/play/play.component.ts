...
export class PlayComponent implements OnInit, OnDestroy {
  ...
  async registerGame(formName, stake_amt, potential_winning) {
    try {
      ...
      console.log('Creating ticket with data:', JSON.stringify(reqData, null, 2)); // More detailed debug log
      console.log('Game ID:', this.game_id);
      console.log('Draw Date:', this.draw_date);
      ...
      if (this.updateData !== undefined) {
        ...
        try {
          ...
          const updateResponse = await this.api.updateAcct(this.updateData).toPromise();
          ...
          // Create the ticket
          const ticketResponse = await this.api.newTicket(reqData).toPromise();
          ...
          try {
            await this.transaction_api.simpleTransaction(newData).toPromise();
          ...
          if (!this.destroy$.closed) {
            this.toastService.success('Ticket created successfully', 7000);
          }

          // Refresh user profile
          const updatedProfile = await this.authApi.refreshCurrentUserProfile();
          ...
          window.scroll(0, 0);
          if (!this.destroy$.closed) {
            this.toastService.error(error.message || 'An error occurred while processing your request');
          }
        }
      }
    } catch (error) {
