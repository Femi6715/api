...
export class MoneymatterService {
  ...
  async updateMainBalance(amount: number): Promise<number> {
    try {
      ...
      // First get the current user data to ensure we have the latest bonus value
      const userResponse = await this.http.get<{ success: boolean; user: any }>(
        `${this.apiUrl}/api/simple/user/${userId}`,
        this.httpOptions
      ).toPromise();
      ...
      const currentBonus = userResponse.user.bonus || 0;
  ...
  verifyPayment(reference: string) {
    return this.http.post<any>(`${this.apiUrl}/api/verify-payment`, { reference }, this.httpOptions);
  }

  revertTransaction(transactionId: string) {
    return this.http.post<any>(`${this.apiUrl}/api/revert-transaction`, { transaction_id: transactionId }, this.httpOptions);
  }
}
