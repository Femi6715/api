...
export class HomeFooterComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    try {
      // Always fetch the latest user profile from backend
      const profile = await this.authService.refreshCurrentUserProfile();
      // Check if profile has error property using proper type checking
      ...
      if (this.user.user_id) {
        this.user.user_id = this.user.user_id.toString();
      }
    } catch (error) {
